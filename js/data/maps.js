...
(function () {
  ...
  // 성 안 건물 4채 - 사용자가 실제 엑셀 파일로 정확히 짚어준 좌표로 보정.
  rectFill(grid, 10, 5, 15, 9, 2);  // 서북 민가(분홍 꽃나무 옆)
  rectFill(grid, 16, 5, 24, 9, 2);  // 중앙 관청(가장 크다)
  grid[9][24] = 0;                  // 관청 남동쪽 모서리 한 칸은 통로로 열어둠
  rectFill(grid, 25, 5, 29, 8, 2);  // 동북 민가
  rectFill(grid, 7, 12, 12, 15, 2); // 서남 주막(홍등)
