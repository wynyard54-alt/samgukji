...
(function () {
  ...
  rectFill(grid, 0, 0, 0, h - 1, 4);
  rectFill(grid, w - 1, 0, w - 1, h - 1, 4);
  rectFill(grid, 7, 0, 8, 0, 0);   // 북쪽 오솔길 입구(쪽문으로 이어짐) - 사용자 확인: x7-8 (기존 7-9에서 한 칸 좁힘)
  ...
  rectFill(grid, 7, 4, 8, 4, 0);    // 서북쪽 쪽문 - 사용자 확인: x7-8 (기존 8-9에서 한 칸 왼쪽으로)
  ...
  // 남동쪽 연못 - 사용자 확인: 기존 사각형(31,21)-(38,26)이 너무 크고 높았음.
  // 그림 속 실제 연못 모양(아래로 갈수록 넓어지는 물가)에 맞춰 계단식으로 축소.
  rectFill(grid, 30, 24, 31, 24, 3);
  rectFill(grid, 30, 25, 32, 25, 3);
  rectFill(grid, 30, 26, 34, 26, 3);
